...
const ProductList = observer(function ProductList() {
    ...
    return (
        <ProductListStoreProvider store={productListStore}>
            <List sx={{ maxWidth: "400px", m: "0 auto" }}>
                ...
                {!!productListStore.products.length && (
                    <Typography sx={{ color: "#888", mb: 4 }}>Last updated on {productListStore.lastUpdatedText}</Typography>
                )}

                {productListStore.loader.isLoading && <Typography>Loading...</Typography>}
                {!productListStore.loader.isLoading &&
                                ...
                                <ListItemText
                                    primary={
                                        <Typography
                                            ...
                                            href={`https://pricespy.co.nz/product.php?p=${product.pricespyId}`}
                                        >
                                            {product.name}
                                        </Typography>
                                    }
